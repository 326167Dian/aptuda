    ...
    switch($_GET['act']){
        ...
        where tgl_trkasir ='$tgl_awal' order by id_trkasir desc ");
            ...
            <div class="box box-primary box-solid table-responsive">
                ...
                <div class="box-body">
                    ...
                    <table id="example1" class="table table-bordered table-striped" >
                        ...
                        <tr>
                            ...
                            <th align="center">Nilai Transaksi</th>
                            <th align="center">Dana Masuk</th>
                            <th width="70">Aksi</th>
                        </tr>
                        </thead>
                        <tbody>
                     ...
                     <!--       <tr>
                        ...
                        <table class="table table-striped table-bordered table-responsive">
                            ...
                            while ($tt=mysqli_fetch_array($tamtot)){
                                ...
                                $tcb= $db->query( "SELECT id_trkasir, kd_trkasir, SUM(nilai_transaksi) as ttlskrg1
                                ...
                                $shift1= $db->query( "SELECT id_trkasir, kd_trkasir, SUM(nilai_transaksi) as ttlskrg2
                                        FROM trkasir WHERE statusbayar='LUNAS' and tgl_trkasir='$tgl_awal' AND id_carabayar='$tt[id_carabayar]' ");
                                $pagi1 = $shift1->fetch_array();
                                $pagi = format_rupiah($pagi1['ttlskrg2']);
                           ...
                           ";
                            ...
                            <?php
                            $tgl_awal = date('Y-m-d');
                            $tothari = $db->query("SELECT id_trkasir, kd_trkasir, SUM(nilai_transaksi) as ttlskrg4                                                              
                                            FROM trkasir WHERE tgl_trkasir='$tgl_awal' ");
                            $arth = $tothari->fetch_array();
                            $tdm = format_rupiah($arth['ttlskrg4']);

                            $totpagi = $db->query("SELECT id_trkasir, kd_trkasir, SUM(nilai_transaksi) as ttlskrg5                                                              
                                            FROM trkasir WHERE tgl_trkasir='$tgl_awal' and statusbayar ='LUNAS' ");
                            $tpagi = $totpagi->fetch_array();
                            $shiftpagi = format_rupiah($tpagi['ttlskrg5']);
                ...
                <a  class ='btn  btn-success btn-flat' href='modul/mod_lapstok/sinkronisasi_stok.php'>SINKRONISASI</a> Klik sinkronisasi setiap tutup shift
            </div>
									...
									<label class='col-sm-4 control-label'>Pelanggan</label> 
									    <div class='col-sm-7'>
    									    <div class='input-group'>
    										    <input type=text name='nm_pelanggan' id='nm_pelanggan' class='form-control' autocomplete='off' readonly>
    										    <div class='input-group-addon'>
    											    <button type=button data-toggle='modal' data-target='#ModalPelanggan' href='#'><span class='glyphicon glyphicon-search'></span></button>
    										    </div>
    										</div>
    									</div>
...
<div id="ModalPelanggan" class="modal fade" role="dialog">
    <div class="modal-lg modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body table-responsive">
                <table id="examplePelanggan" class="table table-bordered table-striped" >
                    <thead>
                        <tr class="judul-table">
                            <th style="vertical-align: middle; background-color: #008000; text-align: center;">No</th>
                            <th style="vertical-align: middle; background-color: #008000; text-align: left; ">Nama Pelanggan</th>
                            <th style="vertical-align: middle; background-color: #008000; text-align: left; ">Telp Pelanggan</th>
                            <th style="vertical-align: middle; background-color: #008000; text-align: left; ">Alamat Pelanggan</th>
                            <th style="vertical-align: middle; background-color: #008000; text-align: left; ">Faktur Jatuh Tempo</th>
                            <th style="vertical-align: middle; background-color: #008000; text-align: center; ">Pilih</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php 
                        $nop=1;
                        $tampil_pelanggan = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pelanggan ORDER BY nm_pelanggan ASC");
                        while ($rp=mysqli_fetch_array($tampil_pelanggan)){
                            $tgl_awal = date('Y-m-d');
                            $tgl_akhir = date('Y-m-d', strtotime('-15 days', strtotime( $tgl_awal)));
				            //cek hutang
                             $hutang = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(id_trkasir) as jml FROM trkasir WHERE nm_pelanggan = '$rp[nm_pelanggan]' and tgl_trkasir <= '$tgl_akhir' AND statusbayar = 'PROSES' ");
                             $jumlah = mysqli_fetch_array($hutang);
                            echo "
                            
                            <tr style='font-size: 13px;'>
                                <td align='center' width='5%'>$nop</td>
                                ";
                                if ($jumlah[jml]>0)
                                {echo "<td align='left' style='background-color:#FF4500;'>$rp[nm_pelanggan]</td>";}
                                else{echo"<td align='left'>$rp[nm_pelanggan]</td>";}
                                
                                echo"
                                <td align='left'>$rp[tlp_pelanggan]</td>
                                <td align='left'>$rp[alamat_pelanggan]</td>
                                <td align='left'>$jumlah[jml]</td>
                                <td align='center' width='5%'>
                                    <button class='btn btn-xs btn-info' id='pilihpelanggan' 
									    data-id_pelanggan='$rp[id_pelanggan]'
										data-nm_pelanggan='$rp[nm_pelanggan]'
										data-tlp_pelanggan='$rp[tlp_pelanggan]'
										data-alamat_pelanggan='$rp[alamat_pelanggan]'
										data-ket_pelanggan='$jumlah[jml]'>
										<i class='fa fa-check'></i>
									</button>
                                </td>
                            </tr>
...
<script>
    ...
    $(document).on('click','#pilihpelanggan', function(){
        document.getElementById('nm_pelanggan').value= $(this).data('nm_pelanggan');
        document.getElementById('tlp_pelanggan').value= $(this).data('tlp_pelanggan');
        document.getElementById('alamat_pelanggan').value= $(this).data('alamat_pelanggan');
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data.result == 'success'){
                            ...
                            alert('Mohon selesaikan pembayaran sebelumnya lebih dahulu !');
                        }
