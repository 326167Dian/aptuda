...
if ($_GET['action'] == "table_data_today") {
    ...
    $from = "FROM trkasir LEFT JOIN carabayar ON trkasir.id_carabayar = carabayar.id_carabayar
        WHERE trkasir.tgl_trkasir = '$today'";
    ...
    if (empty($_POST['search']['value'])) {
        ...
        $extraWhere = " AND (trkasir.kd_trkasir LIKE '%$search%'
        ...
        $query = $db->query("SELECT trkasir.*, carabayar.nm_carabayar $from $extraWhere
            ORDER BY $order $dir LIMIT $limit OFFSET $start");

        $querycount = $db->query("SELECT COUNT(*) as jumlah $from $extraWhere");
        $datacount = $querycount->fetch_array();
    ...
    if (!empty($query)) {
        ...
        while ($value = $query->fetch_array()) {
            ...
            $highlight = '';
            if (in_array($value['id_carabayar'], [2, 3]) && $value['statusbayar'] == 'PROSES') {
                $highlight = 'yellow';
            }

            $aksi_html = "<a class='glyphicon glyphicon-print' onclick=\"window.open('modul/mod_laporan/faktur.php?kd_trkasir=$value[kd_trkasir]','nama window','width=700,height=400,toolbar=no,location=no,directories=no,status=no,menubar=no, scrollbars=no,resizable=yes,copyhistory=no')\">&nbsp</a>";
            if ($level == 'pemilik') {
                $aksi_html = "<a href='?module=trkasir&act=ubah&id=$value[id_trkasir]' title='EDIT' class='glyphicon glyphicon-pencil'>&nbsp</a> "
