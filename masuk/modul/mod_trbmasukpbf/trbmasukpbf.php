    ...
    switch($_GET[act]){
	  ...
	  ORDER BY trbmasuk.id_trbmasuk DESC");
            ...
            <div class="box box-primary box-solid table-responsive">
                ...
                <div class="box-body">
                    <a  class ='btn  btn-success btn-flat' href='?module=trbmasukpbf&act=tambah'>INPUT BARANG MASUK</a><div></div><p><p>
						...
						<div class='col-lg-6'>
							  ...
							  <div class='form-group'>
									...
									<label class='col-sm-4 control-label'>Kode Transaksi</label>        		
										<div class='col-sm-6'>
											<input type=text name='kd_hid' id='kd_hid' class='form-control' required='required' value='$kdtransaksi' autocomplete='off' readonly>
										</div>
						...
						<div class='col-lg-6'>
								...
								<div class='form-group'>
									...
									";
									        ...
									        $jns_obat = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM jenis_obat");
									        while ($jns = mysqli_fetch_array($jns_obat)){
									            echo "<option value='$jns[jenisobat]'>$jns[jenisobat]</option>";
									        }    
								echo "	
										    </select>										
									 </div>
						...
						<div class='col-lg-6'>
							...
							<div class='form-group'>
								...
								<label class='col-sm-4 control-label'>Tanggal</label>
									...
									<label class='col-sm-4 control-label'>Kode Transaksi</label>        		
										<div class='col-sm-6'>
											<input type=text name='kd_hid' id='kd_hid' class='form-control' required='required' value='$re[kd_trbmasuk]' autocomplete='off' readonly>
										</div>
										
									<label class='col-sm-4 control-label'>Supplier</label>        		
										<div class='col-sm-6'>
											<div class='input-group'>
												<input type='text' class='form-control' name='nm_supplier' id='nm_supplier' required='required' value='$re[nm_supplier]' autocomplete='off' readonly>
													<div class='input-group-addon'>
														<button type=button data-toggle='modal' data-target='#ModalSupplier' href='#'><span class='glyphicon glyphicon-search'></span></button>
													</div>
											</div>
...
<script>
			...
			if (e.which == 13) {
				...
				$.ajax({
					...
					for (let i = 0; i < data.length; i++) {
                        ...
                        // document.getElementById('jenisobat').value= data.jenisobat ;
                        ...
                        $('#jns_obat').val(data.jns_obat).change();
	...
	$('#nmbrg_dtrbmasuk_enter').on('click', function(){
	  let nm_barang = $('#nmbrg_dtrbmasuk').val();
				$.ajax({
					...
					// let data = JSON.parse(response)
					let qty_default = "1";
                    // console.log(datas[0].id_barang);
				// 	for (let i = 0; i < datas.length; i++) {
				// 		let data = datas[i];
						
			            document.getElementById('id_barang').value= data[0].id_barang ;
                        document.getElementById('kd_barang').value= data[0].kd_barang ;
                        document.getElementById('nmbrg_dtrbmasuk').value= data[0].nm_barang ;
                        //  document.getElementById('jenisobat').value= data[0].jenisobat ;
                        document.getElementById('stok_barang').value= data[0].stok_barang ;
                        document.getElementById('qty_dtrbmasuk').value= qty_default ;
                        document.getElementById('sat_dtrbmasuk').value= data[0].sat_barang ;
                        document.getElementById('hnasat_dtrbmasuk').value= data[0].hna ;
                        
                        $('#jns_obat').val(data[0].jns_obat).change();
                        document.getElementById('hrgjual_dtrbmasuk').value= formatRupiah(data[0].hrgjual_barang) ;
    ...
    $(document).on('click', '#pilihbarang', function(){
        ...
        $.ajax({
            url: 'modul/mod_trbmasukpbf/gettabel_barang.php',
            type: 'post',
            ...
            success: function(response) {
                ...
                document.getElementById('nmbrg_dtrbmasuk').value= response[0].nm_barang ;
                document.getElementById('stok_barang').value= response[0].stok_barang ;
                document.getElementById('qty_dtrbmasuk').value= qty_default ;
                document.getElementById('sat_dtrbmasuk').value= response[0].sat_barang ;
                document.getElementById('hnasat_dtrbmasuk').value= formatRupiah(response[0].hna_barang) ;
                // document.getElementById('hrgsat_dtrbmasuk').value= formatRupiah(response[0].hrgsat_barang) ;
                    
                // Set the selected option based on its value
                $('#jns_obat').val(response[0].jns_obat).change();
                document.getElementById('hrgjual_dtrbmasuk').value= formatRupiah(response[0].hrgjual_barang) ;
    ...
    function simpan_detail(){
    	...
    	var jns_obat = document.getElementById('jns_obat').value;
